...
| **傳產/原物料/航運** | WTI原油、銅期貨、美元指數、航運ETF(BDRY) | 長榮/中鋼/台塑等權重組合（無單一ETF，用個股組合代理） |
...
首頁另有一個「ETF」頂層分頁，底下再分三個子分頁——因為 ETF 是「投資風格」維度，跟大盤/科技股/金融股/傳產這種「產業」維度不是同一件事，混在同一層會混淆：

| 子分頁 | 因子 | 回測對照標的 |
|--------|------|------|
| **股息型** | 美債殖利率（反向，債券替代效應）、美股金融類股、**美股公用事業ETF(XLU)**（典型債券替代股對照）、外資買賣超、台幣匯率 | 0056.TW（元大高股息） |
| **成長型** | *不重新運算*——直接借用「科技股/半導體」分頁的結果 | 同科技股/半導體 |
| **債券型** | TLT（美國20年期公債，直接代理）、美債殖利率（反向）、VIX（避險效應）、**MOVE指數**（債市專屬波動率，方向與VIX避險版互補） | 00679B.TWO（元大美債20年，上櫃） |

> 市值型（如 0050）與成長型同理不單獨開分頁：0050 台積電權重與大盤相近，套用同一套因子只是大盤的翻版，
> 沒有新的判斷依據。主動型 ETF 也不做——持股由經理人自由裁量、缺乏公開即時持股，無法設計穩定的隔夜訊號。
...
`config.CATEGORIES[key]["backtest_ticker"]` 可以是單一字串（如 `"0052.TW"`），
也可以是字串 list（等權重組合，如傳產類別的 `["2603.TW","2002.TW","1301.TW"]`）——
用於沒有乾淨單一 ETF 可代表的異質類別。
...
- [x] ETF 分頁（股息型/成長型(別名)/債券型，巢狀子分頁）
- [x] 因子缺口研究 + 補上高價值因子（NVDA科技股、XLU股息型、MOVE債券型）
- [ ] 中價值候選（^BKX金融股、SLX鋼鐵傳產、日經指數大盤）視效果評估是否加入
- [ ] 累積 ≥ 10 天資料後校準權重
- [ ] 新聞情緒因子（NLP，第二層）
