...
# 盤中展望門檻（大盤限定）：因子本質是隔夜訊號，對「開盤跳空」預測力遠強於
# 「開盤後盤中走勢」（603 天回測：開盤同向率 94% vs 盤中 71%）。
# |總分| ≥ 此值才對盤中喊方向；3~5 之間 = 只有開盤訊號、盤中不表態。
# T=5 是盤中同向率/覆蓋率的甜蜜點（71%、約半數日子），再高只增門檻不增純度。
THRESHOLD_INTRADAY = 5.0
